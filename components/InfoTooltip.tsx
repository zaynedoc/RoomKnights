...
export const InfoTooltip: React.FC<InfoTooltipProps> = ({ text, direction = 'up' }) => {
  return (
    <span className="group relative inline-flex items-center shrink-0 cursor-help normal-case font-sans">
      <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full border border-[var(--text-muted)] text-[9px] font-bold text-[var(--text-muted)] hover:text-[var(--gold-text)] hover:border-[var(--gold-text)] transition-colors ml-1.5 select-none">
        i
      </span>
